...
SSD_Device::SSD_Device(Device_Parameter_Set *parameters, std::vector<IO_Flow_Parameter_Set *> *io_flows) : MQSimEngine::Sim_Object("SSDDevice")
{
	...
	case NVM::NVM_Type::FLASH:
	{
		...
		// Required for FTL initialization
		...
		case SSD_Components::ONFI_Protocol::NVDDR2:
		{
			...
			for (unsigned int channel_cntr = 0; channel_cntr < parameters->Flash_Channel_Count; channel_cntr++)
			{
				...
				// Channels should not be added to the simulator core, they are
				// passive object that do not handle any simulation event
			}
...
void SSD_Device::ValidateSimulationConfig()
{
}

void SSD_Device::ExecuteSimulatorEvent(MQSimEngine::Sim_Event *event)
{
}

void SSD_Device::ReportResultsInXML(std::string name_prefix, Utils::XmlWriter &xmlwriter)
{
	std::string tmp;
	tmp = ID();
	xmlwriter.WriteOpenTag(tmp);

	this->Host_interface->ReportResultsInXML(ID(), xmlwriter);
	if (Memory_Type == NVM::NVM_Type::FLASH)
	{
		((SSD_Components::FTL *)this->Firmware)->ReportResultsInXML(ID(), xmlwriter);
		((SSD_Components::FTL *)this->Firmware)->TSU->ReportResultsInXML(ID(), xmlwriter);

		for (unsigned int channel_cntr = 0; channel_cntr < Channel_count; channel_cntr++)
		{
			for (unsigned int chip_cntr = 0; chip_cntr < Chip_no_per_channel; chip_cntr++)
			{
				((SSD_Components::ONFI_Channel_NVDDR2 *)Channels[channel_cntr])->Chips[chip_cntr]->ReportResultsInXML(ID(), xmlwriter);
			}
		}
	}
...
unsigned int SSD_Device::GetNoOfLHAsInAnNVMWriteUnit()
{
	return Host_interface->GetNoOfLHAsInAnNVMWriteUnit();
}

LPA_type SSD_Device::ConvertHostLogicToDeviceAddress(LHA_type lha)
{
	return my_instance->Firmware->ConvertHostLogicToDeviceAddress(lha);
}

page_status_type SSD_Device::FindNVMSubunitAccessBitmap(LHA_type lha)
{
	return my_instance->Firmware->FindNVMSubunitAccessBitmap(lha);
}
