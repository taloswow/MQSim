...
public:
	...
	Flow_Type Type;
	IO_Flow_PriorityClass::Priority PriorityClass;
	...
	void XMLSerialize(Utils::XmlWriter& xmlwrite);
...
public:
	...
	unsigned int Working_Set_Percentage;
	...
	// This parameters used if the address distribution type is hot/cold (i.e., (100-H)% 
	// of the whole I/O requests are going to a H% hot region of the storage space)
	...
	// The bandwidth of I/O flow in bytes per second
	// (it should be a multiplication of sector size)

	sim_time_type Stop_Time; // Defines when to stop generating I/O requests
	unsigned int Total_Requests_To_Generate;
	// If Stop_Time is equal to zero, then requst generator considers Total_Requests_To_Generate
	// to decide when to stop generating I/O requests
