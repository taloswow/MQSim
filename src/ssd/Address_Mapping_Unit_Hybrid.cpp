...
{
	Address_Mapping_Unit_Hybrid::Address_Mapping_Unit_Hybrid(sim_object_id_type id,
			FTL* ftl,
			...
			bool fold_out_of_range_addresses) :
		Address_Mapping_Unit_Base(id,
				ftl,
				flash_controller,
				block_manager,
				ideal_mapping_table,
				concurrent_streams_no,
				channel_count,
				chip_no_per_channel,
				die_no_per_chip,
				plane_no_in_die,
	...
	void Address_Mapping_Unit_Hybrid::AllocateAddressForPreconditioning(
			const stream_id_type stream_id,std::map<LPA_type,
	...
	PPA_type Address_Mapping_Unit_Hybrid::OnlineCreateEntryForReads(LPA_type lpa,
			const stream_id_type stream_id,
			NVM::FlashMemory::Physical_Page_Address& read_address,
			uint64_t read_sectors_bitmap) { return 0; }

	bool Address_Mapping_Unit_Hybrid::QueryCMT(NVM_Transaction_Flash* transaction) { return true; }
	...
	bool Address_Mapping_Unit_Hybrid::IsLPALockedForGC(stream_id_type stream_id, LPA_type lpa) { return false; }
	bool Address_Mapping_Unit_Hybrid::IsMVONLockedForGC(stream_id_type stream_id, MVPN_type mvpn) { return false; }
	void Address_Mapping_Unit_Hybrid::ManageUserTransactionFacingBarrier(NVM_Transaction_Flash* transaction) {}
	void Address_Mapping_Unit_Hybrid::ManageMappingTransactionFacingBarrier(stream_id_type stream_id, MVPN_type mvpn, bool read) {}
