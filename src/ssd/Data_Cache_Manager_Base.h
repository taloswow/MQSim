...
namespace SSD_Components
{
	...
	public:
		...
		void ConnectToUserRequestServicedSignal(UserRequestServicedSignalHanderType);
		typedef void(*MemoryTransactionServicedSignalHanderType) (NVM_Transaction*);
		void ConnectToUserMemoryTransactionServicedSignal(MemoryTransactionServicedSignalHanderType);
		void SetHostInterface(Host_Interface_Base* host_interface);
		virtual void DoWarmup(std::vector<Utils::Workload_Statistics*> workload_stats) = 0;
	protected:
		static Data_Cache_Manager_Base* _my_instance;
		Host_Interface_Base* host_interface;
		NVM_Firmware* nvm_firmware;
		unsigned int dram_row_size; // size of the DRAM rows in bytes
		unsigned int dram_data_rate; // in MT/s
		unsigned int dram_busrt_size;
		double dram_burst_transfer_time_ddr;
		// The transfer time of two bursts, changed from sim_time_type to double to increase precision
		...
		void BroadcastUserRequestServicedSignal(User_Request* user_request);
	...
	inline sim_time_type EstimateDRAMAccessTime(const unsigned int memory_access_size_in_byte,
		const unsigned int dram_row_size,
		const unsigned int dram_burst_size_in_bytes,
		const double dram_burst_transfer_time_ddr,
		const sim_time_type tRCD,
		const sim_time_type tCL,
