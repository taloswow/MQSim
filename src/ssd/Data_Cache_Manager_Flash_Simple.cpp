...
{
	Data_Cache_Manager_Flash_Simple::Data_Cache_Manager_Flash_Simple(const sim_object_id_type& id, Host_Interface_Base* host_interface,
	...
	void Data_Cache_Manager_Flash_Simple::ProcessNewUserRequest(User_Request* user_request)
	{
	...
	void Data_Cache_Manager_Flash_Simple::WriteToDestageBuffer(User_Request* user_request)
	{
				...
				case Caching_Mode::WRITE_CACHE:
				{
					...
					for (unsigned int i = 0; i < _my_instance->stream_count; i++) {
						...
						if (((Data_Cache_Manager_Flash_Simple*)_my_instance)->waiting_user_requests_queue_for_dram_free_slot[((Data_Cache_Manager_Flash_Simple*)_my_instance)->request_queue_turn].size() > 0) {
							...
							((Data_Cache_Manager_Flash_Simple*)_my_instance)->WriteToDestageBuffer(*user_request);
							if ((*user_request)->Transaction_list.size() == 0) {
	...
	void Data_Cache_Manager_Flash_Simple::ServiceDRAMAccessRequest(Memory_Transfer_Info* request_info)
	{
	...
	void Data_Cache_Manager_Flash_Simple::ExecuteSimulatorEvent(MQSimEngine::Sim_Event* ev)
	{
		...
		switch (eventType)
			...
			case Data_Cache_Simulation_Event_Type::MEMORY_READ_FOR_CACHE_EVICTION_FINISHED:
				...
				static_cast<FTL*>(nvm_firmware)->Address_Mapping_Unit->TranslateLPAtoPPAAndDispatch(*((std::list<NVM_Transaction*>*)(transfer_inf->Related_request)));
