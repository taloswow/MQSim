...
namespace SSD_Components
{
	Flash_Block_Manager::Flash_Block_Manager(GC_and_WL_Unit_Base* gc_and_wl_unit,
		...
		Flash_Block_Manager_Base(gc_and_wl_unit,
	...
	void Flash_Block_Manager::AllocateBlockAndPageInPlaneForUserWrite(const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		...
		if(plane_record->Data_wf[stream_id]->Current_page_write_index == pages_no_per_block) {
			...
			plane_record->Data_wf[stream_id] = plane_record->GetAFreeBlock(stream_id, false);
			gc_and_wl_unit->CheckGCRequired(plane_record->GetFreeBlockPoolSize(), page_address);
		}

		plane_record->CheckBookkeepingCorrectness(page_address);
	}

	void Flash_Block_Manager::AllocateBlockAndPageInPlaneForGCWrite(const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType *plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Valid_pages_count++;
		plane_record->Free_pages_count--;		
		...
		if (plane_record->GC_wf[stream_id]->Current_page_write_index == pages_no_per_block) {
			// Assign a new write frontier block
			plane_record->GC_wf[stream_id] = plane_record->GetAFreeBlock(stream_id, false);
			gc_and_wl_unit->CheckGCRequired(plane_record->GetFreeBlockPoolSize(), page_address);
		}
		plane_record->CheckBookkeepingCorrectness(page_address);
	}
	
	void Flash_Block_Manager::AllocatePagesInBlockAndInvalidateRemainingForPreconditioning(const stream_id_type stream_id, const NVM::FlashMemory::Physical_Page_Address& plane_address, std::vector<NVM::FlashMemory::Physical_Page_Address>& page_addresses)
	{
		...
		if (plane_record->Data_wf[stream_id]->Current_page_write_index > 0) {
			PRINT_ERROR("Illegal operation: the AllocatePagesInBlockAndInvalidateRemainingForPreconditioning function should be executed for an erased block!")
		}
		...
		for (int i = 0; i < page_addresses.size(); i++) {
			...
			plane_record->CheckBookkeepingCorrectness(page_addresses[i]);
		}
		...
		while (plane_record->Data_wf[stream_id]->Current_page_write_index < pages_no_per_block) {
			...
			InvalidatePageInBlockForPreconditioning(stream_id, target_address);
			plane_record->CheckBookkeepingCorrectness(plane_address);
		}
	...
	void Flash_Block_Manager::AllocateBlockAndPageIinPlaneForTranslationWrite(const stream_id_type streamID, NVM::FlashMemory::Physical_Page_Address& page_address, bool is_for_gc)
	{
		PlaneBookKeepingType *plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Valid_pages_count++;
		plane_record->Free_pages_count--;
		...
		if (plane_record->Translation_wf[streamID]->Current_page_write_index == pages_no_per_block) {
			// Assign a new write frontier block
			plane_record->Translation_wf[streamID] = plane_record->GetAFreeBlock(streamID, true);
			if (!is_for_gc) {
				gc_and_wl_unit->CheckGCRequired(plane_record->GetFreeBlockPoolSize(), page_address);
			}
		}
	...
	inline void Flash_Block_Manager::InvalidatePageInBlock(const stream_id_type stream_id, const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType* plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Invalid_pages_count++;
		plane_record->Valid_pages_count--;
		if (plane_record->Blocks[page_address.BlockID].Stream_id != stream_id) {
			PRINT_ERROR("Inconsistent status in the InvalidatePageInBlock function! The accessed block is not allocated to stream " << stream_id)
		}
	...
	inline void Flash_Block_Manager::InvalidatePageInBlockForPreconditioning(const stream_id_type stream_id, const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType* plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Invalid_pages_count++;
		if (plane_record->Blocks[page_address.BlockID].Stream_id != stream_id) {
	...
	void Flash_Block_Manager::AddErasedBlockToPool(const NVM::FlashMemory::Physical_Page_Address& block_address)
	{
		...
		block->Erase();
		Stats::Block_erase_histogram[block_address.ChannelID][block_address.ChipID][block_address.DieID][block_address.PlaneID][block->Erase_count]++;
		plane_record->AddToFreeBlockPool(block, gc_and_wl_unit->UseDynamicWearleveling());
		plane_record->CheckBookkeepingCorrectness(block_address);
	}

	inline unsigned int Flash_Block_Manager::GetPoolSize(const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
