...
namespace SSD_Components
{
	...
	bool GC_and_WL_Unit_Page_Level::GCIsInUrgentMode(const NVM::FlashMemory::Flash_Chip* chip)
	{
		...
		for (unsigned int die_id = 0; die_id < die_no_per_chip; die_id++) {
			for (unsigned int plane_id = 0; plane_id < plane_no_per_die; plane_id++) {
				...
				if (block_manager->GetPoolSize(addr) < block_pool_gc_hard_threshold)
					return true;
			}
		}
	...
	void GC_and_WL_Unit_Page_Level::CheckGCRequired(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		if (free_block_pool_size < block_pool_gc_threshold) {
			flash_block_ID_type gc_candidate_block_id = block_manager->GetColdestBlockID(plane_address);
			...
			switch (block_selection_policy) {
				case SSD_Components::GC_Block_Selection_Policy_Type::GREEDY:
					// Find the set of blocks with maximum number of invalid pages and no free pages
				{
					...
					for (flash_block_ID_type block_id = 1; block_id < block_no_per_plane; block_id++) {
						if (pbke->Blocks[block_id].Invalid_page_count > pbke->Blocks[gc_candidate_block_id].Invalid_page_count
							...
							&& IsSafeGCWLCandidate(pbke, block_id)) {
				...
				case SSD_Components::GC_Block_Selection_Policy_Type::RANDOM:
				{
					...
					while (!IsSafeGCWLCandidate(pbke, gc_candidate_block_id) && repeat++ < block_no_per_plane) {
						gc_candidate_block_id = random_generator.Uniform_uint(0, block_no_per_plane - 1);
					}
					break;
				}
				case SSD_Components::GC_Block_Selection_Policy_Type::RANDOM_P:
				{
					...
					while ((pbke->Blocks[gc_candidate_block_id].Current_page_write_index < pages_no_per_block || !IsSafeGCWLCandidate(pbke, gc_candidate_block_id))
				...
				case SSD_Components::GC_Block_Selection_Policy_Type::RANDOM_PP:
				{
					...
					while ((pbke->Blocks[gc_candidate_block_id].Current_page_write_index < pages_no_per_block 
						...
						|| !IsSafeGCWLCandidate(pbke, gc_candidate_block_id))
						&& repeat++ < block_no_per_plane) {
						gc_candidate_block_id = random_generator.Uniform_uint(0, block_no_per_plane - 1);
					}
					break;
			...
			address_mapping_unit->SetBarrierForAccessingPhysicalBlock(gc_candidate_address);
