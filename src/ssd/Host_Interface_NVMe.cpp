...
stream_id_type Input_Stream_Manager_NVMe::CreateNewStream(IO_Flow_PriorityClass::Priority priority_class, 
...
inline void Input_Stream_Manager_NVMe::SubmissionQueueTailPointerUpdate(stream_id_type stream_id, uint16_t tail_pointer_value)
{
	...
	{
		((Host_Interface_NVMe *)host_interface)->request_fetch_unit->FetchNextRequest(stream_id);
...
inline void Input_Stream_Manager_NVMe::CompletionQueueHeadPointerUpdate(stream_id_type stream_id, uint16_t head_pointer_value)
{
	...
	if (((Input_Stream_NVMe *)input_streams[stream_id])->Completed_user_requests.size() > 0)
	{
		...
		InformHostRequestCompleted(stream_id, request);
	}
}

inline void Input_Stream_Manager_NVMe::HandleNewArrivedRequest(User_Request *request)
{
	...
	if (request->Type == UserRequestType::READ)
	{
		...
		SegmentUserRequest(request);

		((Host_Interface_NVMe *)host_interface)->BroadcastUserRequestArrivalSignal(request);
	}
	else
	{ // This is a write request
		((Input_Stream_NVMe *)input_streams[request->Stream_id])->Waiting_user_requests.push_back(request);
		((Input_Stream_NVMe *)input_streams[request->Stream_id])->STAT_number_of_write_requests++;
		((Host_Interface_NVMe *)host_interface)->request_fetch_unit->FetchWriteData(request);
	}
}

inline void Input_Stream_Manager_NVMe::HandleArrivedWriteData(User_Request *request)
{
	SegmentUserRequest(request);
	((Host_Interface_NVMe *)host_interface)->BroadcastUserRequestArrivalSignal(request);
}

inline void Input_Stream_Manager_NVMe::HandleServicedRequest(User_Request *request)
{
	...
	if (request->Type == UserRequestType::READ)
	{
		((Host_Interface_NVMe *)host_interface)->request_fetch_unit->SendReadData(request);
	}
	...
	InformHostRequestCompleted(stream_id, request); // Completion queue is not full, so the device can DMA the completion queue entry to the host
	DELETE_REQUEST_NVME(request);
}

uint16_t Input_Stream_Manager_NVMe::GetSubmissionQueueDepth(stream_id_type stream_id)
{
	return ((Input_Stream_NVMe *)this->input_streams[stream_id])->Submission_queue_size;
}

uint16_t Input_Stream_Manager_NVMe::GetCompletionQueueDepth(stream_id_type stream_id)
{
	return ((Input_Stream_NVMe *)this->input_streams[stream_id])->Completion_queue_size;
}

IO_Flow_PriorityClass::Priority Input_Stream_Manager_NVMe::GetPriorityClass(stream_id_type stream_id)
{
	return ((Input_Stream_NVMe *)this->input_streams[stream_id])->Priority_class;
}

inline void Input_Stream_Manager_NVMe::InformHostRequestCompleted(stream_id_type stream_id, User_Request *request)
{
	((Request_Fetch_Unit_NVMe *)((Host_Interface_NVMe *)host_interface)->request_fetch_unit)->SendCompletionQueueElement(request, ((Input_Stream_NVMe *)input_streams[stream_id])->Submission_head_informed_to_host);
...
void Input_Stream_Manager_NVMe::SegmentUserRequest(User_Request *user_request)
{
	...
	while (handled_sectors_count < req_size)
	{
		...
		LHA_type internal_lsa = lsa - ((Input_Stream_NVMe *)input_streams[user_request->Stream_id])->Start_logical_sector_address;
...
void Request_Fetch_Unit_NVMe::ProcessPCIeWriteMessage(uint64_t address, void *payload, unsigned int payload_size)
{
	...
	case SUBMISSION_QUEUE_REGISTER_1:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->SubmissionQueueTailPointerUpdate(0, (uint16_t)val);
		break;
	case COMPLETION_QUEUE_REGISTER_1:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->CompletionQueueHeadPointerUpdate(0, (uint16_t)val);
		break;
	case SUBMISSION_QUEUE_REGISTER_2:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->SubmissionQueueTailPointerUpdate(1, (uint16_t)val);
		break;
	case COMPLETION_QUEUE_REGISTER_2:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->CompletionQueueHeadPointerUpdate(1, (uint16_t)val);
		break;
	case SUBMISSION_QUEUE_REGISTER_3:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->SubmissionQueueTailPointerUpdate(2, (uint16_t)val);
		break;
	case COMPLETION_QUEUE_REGISTER_3:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->CompletionQueueHeadPointerUpdate(2, (uint16_t)val);
		break;
	case SUBMISSION_QUEUE_REGISTER_4:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->SubmissionQueueTailPointerUpdate(3, (uint16_t)val);
		break;
	case COMPLETION_QUEUE_REGISTER_4:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->CompletionQueueHeadPointerUpdate(3, (uint16_t)val);
		break;
	case SUBMISSION_QUEUE_REGISTER_5:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->SubmissionQueueTailPointerUpdate(4, (uint16_t)val);
		break;
	case COMPLETION_QUEUE_REGISTER_5:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->CompletionQueueHeadPointerUpdate(4, (uint16_t)val);
		break;
	case SUBMISSION_QUEUE_REGISTER_6:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->SubmissionQueueTailPointerUpdate(5, (uint16_t)val);
		break;
	case COMPLETION_QUEUE_REGISTER_6:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->CompletionQueueHeadPointerUpdate(5, (uint16_t)val);
		break;
	case SUBMISSION_QUEUE_REGISTER_7:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->SubmissionQueueTailPointerUpdate(6, (uint16_t)val);
		break;
	case COMPLETION_QUEUE_REGISTER_7:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->CompletionQueueHeadPointerUpdate(6, (uint16_t)val);
		break;
	case SUBMISSION_QUEUE_REGISTER_8:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->SubmissionQueueTailPointerUpdate(7, (uint16_t)val);
		break;
	case COMPLETION_QUEUE_REGISTER_8:
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->CompletionQueueHeadPointerUpdate(7, (uint16_t)val);
		break;
	default:
...
void Request_Fetch_Unit_NVMe::ProcessPCIeReadMessage(uint64_t address, void *payload, unsigned int payload_size)
{
	Host_Interface_NVMe *hi = (Host_Interface_NVMe *)host_interface;
	...
	case DMA_Req_Type::REQUEST_INFO:
	{
		...
		new_request->Priority_class = ((Input_Stream_Manager_NVMe *)host_interface->input_stream_manager)->GetPriorityClass(new_request->Stream_id);
		...
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->HandleNewArrivedRequest(new_request);
		break;
	}
	case DMA_Req_Type::WRITE_DATA:
		COPYDATA(((User_Request *)dma_req_item->object)->Data, payload, payload_size);
		((Input_Stream_Manager_NVMe *)(hi->input_stream_manager))->HandleArrivedWriteData((User_Request *)dma_req_item->object);
		break;
	default:
		break;
	}
...
void Request_Fetch_Unit_NVMe::FetchNextRequest(stream_id_type stream_id)
{
	...
	host_interface->SendReadMessageToHost(im->Submission_queue_base_address + im->Submission_head * sizeof(Submission_Queue_Entry), sizeof(Submission_Queue_Entry));
}

void Request_Fetch_Unit_NVMe::FetchWriteData(User_Request *request)
{
	DMA_Req_Item *dma_req_item = new DMA_Req_Item;
	...
	host_interface->SendReadMessageToHost((sqe->PRP_entry_2 << 31) | sqe->PRP_entry_1, request->Size_in_byte);
}

void Request_Fetch_Unit_NVMe::SendCompletionQueueElement(User_Request *request, uint16_t sq_head_value)
{
	Host_Interface_NVMe *hi = (Host_Interface_NVMe *)host_interface;
	...
	host_interface->SendWriteMessageToHost(im->Completion_queue_base_address + im->Completion_tail * sizeof(Completion_Queue_Entry), cqe, sizeof(Completion_Queue_Entry));
	number_of_sent_cqe++;
...
void Request_Fetch_Unit_NVMe::SendReadData(User_Request *request)
{
	Submission_Queue_Entry *sqe = (Submission_Queue_Entry *)request->IO_command_info;
	host_interface->SendWriteMessageToHost(sqe->PRP_entry_1, request->Data, request->Size_in_byte);
}
...
stream_id_type Host_Interface_NVMe::CreateNewStream(IO_Flow_PriorityClass::Priority priority_class, LHA_type start_logical_sector_address, LHA_type end_logical_sector_address,
													  uint64_t submission_queue_base_address, uint64_t completion_queue_base_address)
{
	return ((Input_Stream_Manager_NVMe *)input_stream_manager)->CreateNewStream(priority_class, start_logical_sector_address, end_logical_sector_address, submission_queue_base_address, submission_queue_depth, completion_queue_base_address, completion_queue_depth);
}
...
uint16_t Host_Interface_NVMe::GetSubmissionQueueDepth()
{
	return submission_queue_depth;
}

uint16_t Host_Interface_NVMe::GetCompletionQueueDepth()
{
...
void Host_Interface_NVMe::ReportResultsInXML(std::string name_prefix, Utils::XmlWriter &xmlwriter)
{
	...
	for (unsigned int stream_id = 0; stream_id < no_of_input_streams; stream_id++)
	{
		...
		val = std::to_string(input_stream_manager->GetAverageReadTransactionTurnaroundTime(stream_id));
		xmlwriter.WriteAttributeString(attr, val);

		attr = "Average_Read_Transaction_Execution_Time";
		val = std::to_string(input_stream_manager->GetAverageReadTransactionExecutionTime(stream_id));
		xmlwriter.WriteAttributeString(attr, val);

		attr = "Average_Read_Transaction_Transfer_Time";
		val = std::to_string(input_stream_manager->GetAverageReadTransactionTransferTime(stream_id));
		xmlwriter.WriteAttributeString(attr, val);

		attr = "Average_Read_Transaction_Waiting_Time";
		val = std::to_string(input_stream_manager->GetAverageReadTransactionWaitingTime(stream_id));
		xmlwriter.WriteAttributeString(attr, val);

		attr = "Average_Write_Transaction_Turnaround_Time";
		val = std::to_string(input_stream_manager->GetAverageWriteTransactionTurnaroundTime(stream_id));
		xmlwriter.WriteAttributeString(attr, val);

		attr = "Average_Write_Transaction_Execution_Time";
		val = std::to_string(input_stream_manager->GetAverageWriteTransactionExecutionTime(stream_id));
		xmlwriter.WriteAttributeString(attr, val);

		attr = "Average_Write_Transaction_Transfer_Time";
		val = std::to_string(input_stream_manager->GetAverageWriteTransactionTransferTime(stream_id));
		xmlwriter.WriteAttributeString(attr, val);

		attr = "Average_Write_Transaction_Waiting_Time";
		val = std::to_string(input_stream_manager->GetAverageWriteTransactionWaitingTime(stream_id));
		xmlwriter.WriteAttributeString(attr, val);
