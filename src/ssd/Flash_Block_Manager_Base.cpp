...
namespace SSD_Components
{
				...
				for (unsigned int dieID = 0; dieID < die_no_per_chip; dieID++) {
					...
					for (unsigned int planeID = 0; planeID < plane_no_per_die; planeID++) {
						...
						for (unsigned int blockID = 0; blockID < block_no_per_plane; blockID++) {
							...
							plane_manager[channelID][chipID][dieID][planeID].AddToFreeBlockPool(&plane_manager[channelID][chipID][dieID][planeID].Blocks[blockID], false);
						}
						...
						for (unsigned int stream_cntr = 0; stream_cntr < total_concurrent_streams_no; stream_cntr++) {
							plane_manager[channelID][chipID][dieID][planeID].Data_wf[stream_cntr] = plane_manager[channelID][chipID][dieID][planeID].GetAFreeBlock(stream_cntr, false);
							plane_manager[channelID][chipID][dieID][planeID].Translation_wf[stream_cntr] = plane_manager[channelID][chipID][dieID][planeID].GetAFreeBlock(stream_cntr, true);
							plane_manager[channelID][chipID][dieID][planeID].GC_wf[stream_cntr] = plane_manager[channelID][chipID][dieID][planeID].GetAFreeBlock(stream_cntr, false);
						}
					}
				}
			}
	...
	void Flash_Block_Manager_Base::SetGCAndWLUnit(GC_and_WL_Unit_Base* gcwl)
	{
	...
	Block_Pool_Slot_Type* PlaneBookKeepingType::GetAFreeBlock(stream_id_type stream_id, bool for_mapping_data)
	{
	...
	void PlaneBookKeepingType::CheckBookkeepingCorrectness(const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		...
		if (Free_pages_count == 0) {
			PRINT_ERROR("Plane " << "@" << plane_address.ChannelID << "@" << plane_address.ChipID << "@" << plane_address.DieID << "@" << plane_address.PlaneID << " pool size: " << GetFreeBlockPoolSize() << " ran out of free pages! Bad resource management! It is not safe to continue simulation!");
		}
	}

	unsigned int PlaneBookKeepingType::GetFreeBlockPoolSize()
	{
		return (unsigned int)Free_block_pool.size();
	}

	void PlaneBookKeepingType::AddToFreeBlockPool(Block_Pool_Slot_Type* block, bool consider_dynamic_wl)
	{
	...
	unsigned int Flash_Block_Manager_Base::GetMinMaxEraseDifference(const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
	...
	flash_block_ID_type Flash_Block_Manager_Base::GetColdestBlockID(const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		unsigned int min_erased_block = 0;
		PlaneBookKeepingType *plane_record = &plane_manager[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID];
	...
	PlaneBookKeepingType* Flash_Block_Manager_Base::GetPlaneBookkeepingEntry(const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		return &(plane_manager[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID]);
	}

	bool Flash_Block_Manager_Base::BlockHasOngoingGCWL(const NVM::FlashMemory::Physical_Page_Address& block_address)
	{
	...
	bool Flash_Block_Manager_Base::CanExecuteGCWL(const NVM::FlashMemory::Physical_Page_Address& block_address)
	{
		PlaneBookKeepingType *plane_record = &plane_manager[block_address.ChannelID][block_address.ChipID][block_address.DieID][block_address.PlaneID];
	...
	void Flash_Block_Manager_Base::ProgramTransactionIssued(const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType *plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Blocks[page_address.BlockID].Ongoing_user_program_count++;
	}
	
	void Flash_Block_Manager_Base::ReadTransactionIssued(const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType *plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Blocks[page_address.BlockID].Ongoing_user_read_count++;
	}

	void Flash_Block_Manager_Base::ProgramTransactionServiced(const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType *plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Blocks[page_address.BlockID].Ongoing_user_program_count--;
	}

	void Flash_Block_Manager_Base::ReadTransactionServiced(const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType *plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Blocks[page_address.BlockID].Ongoing_user_read_count--;
	}
	...
	bool Flash_Block_Manager_Base::IsPageValid(Block_Pool_Slot_Type* block, flash_page_ID_type page_id)
	{
