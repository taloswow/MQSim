		...
		if (address >= DATA_MEMORY_REGION) {
			...
			switch (SSD_device_type) {
				case HostInterface_Types::NVME:
				{
					...
					((*IO_flows)[flow_id])->NVMeConsumeIORequest((Completion_Queue_Entry*)payload);
					break;
				}
				case HostInterface_Types::SATA:
					sata_hba->SATAConsumeIORequest((Completion_Queue_Entry*)payload);
					break;
				default:
		...
		if (address >= DATA_MEMORY_REGION) {
			...
			switch (SSD_device_type) {
				case HostInterface_Types::NVME:
				{
					...
					new_pcie_message->Payload = (*IO_flows)[flow_id]->NVMeReadSqe(address);
