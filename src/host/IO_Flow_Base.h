...
#define NVME_UPDATE_SQ_TAIL(Q)  Q.Submission_queue_tail++;\
	...
	public:
		...
		IO_Flow_PriorityClass::Priority Priority_class() { return priority_class; }
		virtual Host_IO_Request* GenerateNextRequest() = 0;
		virtual void NVMeConsumeIORequest(Completion_Queue_Entry*);
		Submission_Queue_Entry* NVMeReadSqe(uint64_t address);
		const NVMe_Queue_Pair* GetNVMeQueuePairInfo();
		virtual void SATAConsumeIORequest(Host_IO_Request* request);
		...
		virtual void GetStatistics(Utils::Workload_Statistics& stats, LPA_type(*ConvertHostLogicToDeviceAddress)(LHA_type lha),
			page_status_type(*FindNVMSubunitAccessBitmap)(LHA_type lha)) = 0;
	protected:
		uint16_t flow_id;
		double initial_occupancy_ratio;
		...
		// If stop_time is zero, then the flow stops generating request when the number
		// of generated requests is equal to total_req_count
		...
		void SubmitIORequest(Host_IO_Request*);
