...
namespace Host_Components
{
	...
	public:
		IO_Flow_Synthetic(const sim_object_id_type &name,
				...
				int seed, sim_time_type stop_time,
				double initial_occupancy_ratio,
				unsigned int total_req_count,
		...
		~IO_Flow_Synthetic();
		Host_IO_Request *GenerateNextRequest();
		void NVMeConsumeIORequest(Completion_Queue_Entry *);
		void SATAConsumeIORequest(Host_IO_Request *);
		void StartSimulation();
		void ValidateSimulationConfig();
		void ExecuteSimulatorEvent(MQSimEngine::Sim_Event *);
		void GetStatistics(Utils::Workload_Statistics &stats, 
				LPA_type (*ConvertHostLogicToDeviceAddress)(LHA_type lha),
