...
namespace NVM
{
	namespace FlashMemory
	{
		Flash_Chip::Flash_Chip(const sim_object_id_type& id,
				...
				sim_time_type* readLatency,
				sim_time_type* programLatency,
		...
		void Flash_Chip::ConnectToChipReadySignal(ChipReadySignalHandlerType function)
		{
		...
		void Flash_Chip::ChangeMemoryStatusPreconditioning(const NVM_Memory_Address* address, const void* status_info)
		{
		...
		void Flash_Chip::SetupTriggers()
		{
			MQSimEngine::Sim_Object::SetupTriggers();
		}
		...
		LPA_type Flash_Chip::GetMetadata(flash_die_ID_type die_id, flash_plane_ID_type plane_id, flash_block_ID_type block_id, flash_page_ID_type page_id)
		// A simplification to decrease the complexity of GC execution!
		// The GC unit may need to know the metadata of a page to decide if a page is valid or invalid. 
		{
		...
		void Flash_Chip::StartCommandExecution(Flash_Command* command)
		{
			...
			targetDie->Expected_finish_time = Simulator->Time() + GetCommandExecutionLatency(command->CommandCode, command->Address[0].PageID);
		...
		void Flash_Chip::FinishCommandExecution(Flash_Command* command)
		{
			Die* targetDie = Dies[command->Address[0].DieID];

			targetDie->STAT_TotalReadTime += GetCommandExecutionLatency(command->CommandCode, command->Address[0].PageID);
			...
			switch (command->CommandCode)
			{
				...
				case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
					...
					for (unsigned int planeCntr = 0; planeCntr < command->Address.size(); planeCntr++) {
						...
						targetDie->Planes[command->Address[planeCntr].PlaneID]->Blocks[command->Address[planeCntr].BlockID]->Pages[command->Address[planeCntr].PageID].ReadMetadata(command->Meta_data[planeCntr]);
					}
					break;
				...
				case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
					...
					for (unsigned int planeCntr = 0; planeCntr < command->Address.size(); planeCntr++) {
						...
						targetDie->Planes[command->Address[planeCntr].PlaneID]->Blocks[command->Address[planeCntr].BlockID]->Pages[command->Address[planeCntr].PageID].WriteMetadata(command->Meta_data[planeCntr]);
					}
					break;
		...
		void Flash_Chip::BroadcastReadySignal(Flash_Command* command)
		{
		...
		void Flash_Chip::Suspend(flash_die_ID_type dieID)
		{
			...
			Simulator->IgnoreSimEvent(targetDie->CommandFinishEvent);
			// The simulator engine should not execute the finish event for the suspended command
			targetDie->CommandFinishEvent = NULL;
